...
const uri = `mongodb+srv://${process.env.DB_NAME}:${process.env.DB_PASS}@cluster0.fs0mclr.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post('/chocolate', async(req, res) => {
        ...
        const result = chocolateCollection.insertOne(newChocolate);
        res.send(result)
    })
...
app.listen(port, () => {
    console.log(`chocolate server running on port: ${port}`);
})